...
class LabelCb(QLabel):
	
	def __init__(self, sText, releaseCb=None, moveCb=None, move_release_cb=None):
		self.m_releaseCb = CFunctor(releaseCb)
		self.m_moveCb = CFunctor(moveCb)
		self.m_move_release_cb = CFunctor(move_release_cb)
		super().__init__(sText)
	...
	def adjustSizeToText(self):
		...
		self.setFixedSize(textWidth + 40, textHeight + 20)
	
	def mouseReleaseEvent(self, QMouseEvent):
		if Qt.LeftButton:
			if self.m_bMoving:
				self.m_bMoving = False
				if self.m_move_release_cb:
					self.m_move_release_cb()
			elif self.m_releaseCb:
				self.m_releaseCb()
			QMouseEvent.accept()
	...
	def mouseMoveEvent(self, QMouseEvent):
		if Qt.LeftButton:
			self.m_bMoving = True
			if self.m_moveCb:
				self.m_moveCb(QMouseEvent.globalPos())
			QMouseEvent.accept()
